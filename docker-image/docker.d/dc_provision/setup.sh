...
samba-tool domain provision --root=${INIT_DOMAIN} --domain=${SAMBA_workgroup} --host-name=${HOSTNAME} --host-ip=${INIT_DC_IP} --adminpass=${INIT_PASS} --dns-backend=BIND9_DLZ --server-role=dc --realm=${SAMBA_realm}
